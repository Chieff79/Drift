# Privacy Policy — Drift

**Last updated: March 2026**

## Overview

Drift ("we", "our", "the app") is committed to protecting your privacy. This policy explains what data we collect and how we use it.

## Data We Do NOT Collect

- We do **not** log your browsing history or traffic content
- We do **not** store your IP address
- We do **not** sell your data to third parties
- We do **not** require account registration

## Data We Collect

### Crash Reports (Optional)
If you enable analytics, we collect anonymous crash reports via Sentry to improve app stability. This includes:
- App version and platform (iOS/Android)
- Crash stack traces (no personal data)

You can disable this in Settings → Analytics.

### Secure Connection
- Your traffic passes through our servers (NL, RU, US, KZ)
- We do not log traffic content or destinations
- Temporary connection metadata (timestamps) may be kept for up to 24 hours for abuse prevention

## Third-Party Services

- **Cloudflare WARP** — if you enable WARP, Cloudflare's [privacy policy](https://www.cloudflare.com/privacypolicy/) applies
- **Apple TestFlight** — governed by Apple's privacy policy

## Contact

For privacy questions: [@driftvpn](https://t.me/driftvpn)
